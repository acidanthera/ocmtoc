...
struct subsystem_argument subsystem_arguments[] = {
    ...
    { "DXE_SAL_DRIVER",		IMAGE_SUBSYSTEM_SAL_RUNTIME_DRIVER },
    { "SAL_RT_DRIVER",		IMAGE_SUBSYSTEM_SAL_RUNTIME_DRIVER },
	...
	for(i = 0; i < arch->object->mh->ncmds; i++){
		...
		if(strcmp(sg->segname, SEG_TEXT) == 0)
		  ...
		  reloc_fix_vmaddr = sg->vmaddr;
		    ...
		    nscns++;
		else if(strcmp(sg->segname, "__SBAT") == 0)
    	    nscns++;
	...
	for(i = 0; i < arch->object->mh64->ncmds; i++){
		...
		if(strcmp(sg64->segname, SEG_TEXT) == 0)
		  ...
		  reloc_fix_vmaddr = sg64->vmaddr;
		    ...
		    nscns++;
		else if(strcmp(sg64->segname, "__SBAT") == 0)
    	    nscns++;
